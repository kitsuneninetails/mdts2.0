...
class NetworkHost(Host):
    ...
    def prepareFiles(self):
        ...
        LinuxCLI().regex_file('/usr/share/midonet-api/WEB-INF/web.xml', 
                            's/\(127.0.0.1:2181\|' + str(self.zookeeper_ips[0][0]) + \
                            ':2181[^<]*\)/' + ip_str + '/')

        if LinuxCLI().grep_file('/usr/share/midonet-api/WEB-INF/web.xml', 'zookeeper-curator_enabled'):
            LinuxCLI().regex_file('/usr/share/midonet-api/WEB-INF/web.xml',
        ...
        LinuxCLI().write_to_file('/etc/tomcat7/Catalina/localhost/midonet-api.xml', tcatcfg, False)

        if not LinuxCLI().grep_file("/etc/default/tomcat7", "java.security.egd"):
            LinuxCLI().regex_file('/etc/default/tomcat7', 
                                '$aJAVA_OPTS="$JAVA_OPTS -Djava.security.egd=file:/dev/./urandom"')

        if LinuxCLI().exists('/var/www/html/midonet-cp/config.js'):
            LinuxCLI().regex_file('/var/www/html/midonet-cp/config.js',
                                ('s%https://example.com/midonet-api%http://$public:8080/midonet-api%'
                                'g;s/example.com/$public:8443/g'))
        elif LinuxCLI().exists('/var/www/midonet-cp/config.js'):
            LinuxCLI().regex_file('/var/www/midonet-cp/config.js',
                                ('s%https://example.com/midonet-api%http://$public:8080/midonet-api%'
                                'g;s/example.com/$public:8443/g'))
